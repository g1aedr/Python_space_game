...
class Enemy(Ship):
    ...
    def ai(self):
        try:
            ...
            pass  # fallback se l'API non è disponibile
...
while running:
    ...
    # 🔁 INVIA POSIZIONE GIOCATORE ALL'API
    try:
        requests.post("http://127.0.0.1:5000/position", json={
            "x": player.x,
            "y": player.y,
            "angle": player.angle
        })
    ...
    for enemy in enemies:
        enemy.ai()  # senza passare player, ora prende dati dall'API
    ...
    # Collisioni, controllo vittoria/sconfitta, disegno, ecc.
